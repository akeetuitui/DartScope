...
봇 명령어는 `/start`, `/help`를 지원합니다. 일반 메시지는 기업명, 사업연도, 선택 단위, `텍스트` 요청을 해석합니다. 봇은 연결재무제표 기준으로 매출액, 영업이익, 영업이익률, 당기순이익, 부채비율, ROE, ROA를 답장합니다. 여러 연도를 보내면 연도별 비교 형태로 답장합니다.
...
웹에서는 `사업보고서 텍스트도 보기`를 체크하면 최신 입력 연도 기준으로 아래 섹션을 추출해 보여줍니다.
